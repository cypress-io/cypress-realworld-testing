...
export default function HomeSteps({ progressState, learnJson }) {
  var settings = {
    dots: false,
    infinite: false,
    speed: 500,
  ...
  return (
    <nav aria-label="Progress">
      <ol className="overflow-hidden">
        {learnJson.map((section, index) => (
          <li
            key={index}
            className={classNames(
              index !== learnJson.length - 1 ? "pb-10" : "",
              "relative"
            )}
          >
            {/* Solid Line that connects the checkmarks */}
            {index !== learnJson.length - 1 ? (
              <div
                className={`-ml-px absolute mt-0.5 top-4 left-4 w-0.5 h-full ${
                  learnJson[index].status === "completed"
                    ? "bg-indigo-600"
                    : "bg-gray-300"
                }`}
                aria-hidden="true"
            ...
            {/* "Current" */}
            {learnJson[index].status === "current" && (
