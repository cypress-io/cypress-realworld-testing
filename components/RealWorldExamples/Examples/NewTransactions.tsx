...
export default function ExamplesLayout({ examples }) {
  return (
    <section data-test={`real-world-example-new-transactions`}>
      <div className={`grid grid-cols-2 gap-8 p-16 h-full bg-indigo-100`}>
        <div className="w-6/12 mx-auto">
          <h3
            className={`mt-2 text-3xl font-extrabold text-gray-900 tracking-tight sm:text-4xl mb-8`}
            data-test={`new-transactions-title`}
          >
            {examples.title}
          </h3>
      ...
      {/* Examples */}
      <div
        className={`relative pb-20 px-4 sm:px-6 lg:pt-2 lg:pb-28 lg:px-8 bg-indigo-100`}
      >
        <div className="absolute inset-0">
          <div className={`h-1/3 sm:h-2/3 bg-indigo-100`} />
        </div>
            ...
            {examples.lessons.map((example, indx) => (
              ...
              >
                <a className="flex flex-col rounded-lg shadow-lg overflow-hidden">
                  <div className={`p-8 text-center bg-indigo-300`}>
                    <Image
                      className="mx-auto"
                  ...
                  <div className="flex-1 p-6 flex flex-col justify-between">
                    <div className="flex-1">
                      <p
                        ...
                        data-test={`new-transactions-${indx}-title`}
                      >
                        {example.title}
                      </p>
                      <p
                        className={`mt-3 text-base text-gray-600`}
                        data-test={`new-transactions-${indx}-description`}
                      >
                        {example.description}
                      </p>
                    </div>
