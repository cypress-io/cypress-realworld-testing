...
Since we are using TypeScript, we must add the type definition for our custom command. We wrap the declaration in `declare global` and extend the `Chainable` interface within the `Cypress` namespace. Because `cypress/support/commands.ts` is a module, `declare global` is what makes the type augmentation apply globally, which allows us to use (and provide code completion) for the `getByData()` method off of the `cy` object. This matches the [approach recommended in the Cypress docs](https://docs.cypress.io/api/cypress-api/custom-commands#TypeScript).
...
<YouTube id="5F7t9IPoUvo" title="Real World Testing with Cypress - custom cypress commands"/>
