...
Instead, we can use [contains](https://docs.cypress.io/api/commands/eq) method that we also used previously. We can look for the text 'Getting started', which will select the `<a>` tag we want:
