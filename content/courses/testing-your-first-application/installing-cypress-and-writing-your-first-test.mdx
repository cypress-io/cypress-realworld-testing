...
Cypress gives us the ability to write E2E or component tests. In this course, we are going to focus solely on E2E tests.
...
Click on the “Create new spec” button.
...
Then click on “Chrome” and the “Start E2E Testing in Chrome” button.
...
Cypress lets us set a global `baseUrl` so we only have to define the address of our application in one place. Setting a `baseUrl` is also a [Cypress best practice](https://docs.cypress.io/app/core-concepts/best-practices#Setting-a-Global-baseUrl): in addition to removing the hardcoded URLs, it allows us to easily switch between environments (for example, local development vs. a deployed staging server) and it speeds up the very first command in our tests by removing an initial page reload.
...
Now that Cypress knows the base address of our application, we can pass a relative path to `cy.visit()` instead of the full URL. To visit the home page, we simply pass `"/"`:
...
In this lesson, you learned how to install Cypress and configure it for E2E testing. You also learned how to create spec files and write Cypress tests. Along the way, you learned some Cypress best practices for getting elements, including setting a global `baseUrl` so you can visit relative paths instead of hardcoding full URLs. Finally, you learned how to refactor and clean up your tests using a beforeEach hook.
