...
Finally, inside the `it()` body we have [cy.visit()](https://on.cypress.io/visit).

[visit](https://on.cypress.io/visit) tells Cypress which URL to load before running the test. We will run our application locally at [localhost:3000](https://on.cypress.io/visit), so update `cy.visit()` with that address.
...
A page should only have a single `h1` when following SEO best practices, which is true of our application. That means we can grab it with the [.get()](https://on.cypress.io/get) command:
...
Then we [chain](https://learn.cypress.io/cypress-fundamentals/command-chaining) the [contains](https://on.cypress.io/contains) command onto it, passing in the text we expect to find inside the `h1`.
...
If you read the [get command API documentation](https://on.cypress.io/get), you'll see it accepts many things: HTML elements, classes like `.button`, IDs like `#button`, and more. At Cypress, we believe you should target elements using things that don't change.
...
:::tip
To learn more, read our best practices on the docs site [here](https://on.cypress.io/best-practices#Selecting-Elements).
:::
...
Since we're getting back a list, we can use the [eq](https://on.cypress.io/eq) command to access a specific index in that list, which is exactly what we need.
