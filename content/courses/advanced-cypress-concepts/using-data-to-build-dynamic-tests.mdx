...
In this example, `cy.database()` is a custom Cypress command which performs operations on our database depending upon which arguments are passed into it. You can learn how [the `cy.database()` command works](/real-world-examples/custom-cypress-commands#cydatabase).
