...
Sometimes you may need to override an already existing intercept by using a **fixture** to **mock** the response data. Let's look at an example from the [Real World App](https://github.com/cypress-io/cypress-realworld-app/blob/develop/cypress/tests/ui/transaction-feeds.spec.ts).
...
If you would like to practice intercepting Network Requests and working with the Network in general with Cypress, we have created the [cypress-realworld-testing-blog repository](https://github.com/cypress-io/cypress-realworld-testing-blog). The installation instructions are located in the **README.md** file.
