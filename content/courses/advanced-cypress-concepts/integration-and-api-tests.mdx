...
One of the pleasant aspects of using Cypress for testing APIs is that you can easily see and interact with the response data from within the Cypress App.

Clicking upon the request step in the Cypress Command Log, we can see all of the response data in the browser's console.

![Screenshot of browser testing in the Cypress App showing a api-users.spec.ts file running. The request command is higlighted in purple in the Command Log. The devtools are open displaying the request and response data from the request.](/images/advanced-cypress-concepts/integration-and-api-tests/Screen_Shot_2021-09-01_at_9.20.04_AM.png)
