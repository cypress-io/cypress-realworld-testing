...
In this lesson, we will learn how to integrate [Cypress Cloud](https://www.cypress.io/cloud/) with GitHub Actions so that our tests will run in parallel. Cypress Cloud will also provide us with greater insights into our tests which will come in handy anytime our tests fail.

You will need to signup for a free account over at [cypress.io/cloud](https://www.cypress.io/cloud/). We recommend using your GitHub account to register.
...
Next, we will need to copy the `projectID` into our `cypress.config.ts` file.
...
We also need to add `record: true` underneath the `wait-on:` directive. This will tell Cypress Cloud to record our test runs.
...
Note that we are passing the `GITHUB_TOKEN` as well. This does not need to be in our GitHub secrets, GitHub will handle this for us. It is used by Cypress Cloud to detect a build vs a re-run build.
...
If you would like to see how Cypress Cloud integrates with a real-world application, check out the [Real World App](https://github.com/cypress-io/cypress-realworld-app) created by the Cypress team. The [Real World App](https://github.com/cypress-io/cypress-realworld-app) (RWA) is a payment application that demonstrates **real-world** usage of Cypress testing methods, patterns, and workflows. Cypress Cloud for this application is open to the public so feel free to inspect any PR to see how it integrates with Cypress Cloud.
