...
Within the `cypress.config.ts` file in the root of the repo, add the following:
...
In this lesson, we learned how to install Cypress and how to write several different tests to ensure that our store is working as expected. We also learned how to customize the Cypress test runner within the `cypress.config.ts` file as well has how to use custom Cypress commands.
